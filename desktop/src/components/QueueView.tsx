...
export function QueueView({ jobs, retry, clear }: { jobs: UploadJob[]; retry(id: string): void; clear(): void }) {
  ...
  return <div className="queue-page">
    ...
    {jobs.length === 0 ? <div className="home-empty"><span className="pixel-face"><Icon name="sad" /></span><h2>nothing here yet</h2><p>run the plugin in studio. uploads show up here.</p></div> : <div className="clean-list">{jobs.map((job) => <article className="clean-job" key={job.id}>
      ...
      {job.status === 'failed' && <button className="icon-button" aria-label="Retry" onClick={() => retry(job.id)}><Icon name="retry" /></button>}
    </article>)}</div>}
  </div>;
}
